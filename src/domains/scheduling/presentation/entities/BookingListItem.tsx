...
import { CLinkText, CText } from '~shared/presentation/ui';
import { StatusButton } from '~shared/presentation/components/StatusButton';
...
import {
	...
	type Booking,
	type BookingStatus,
} from '~domains/scheduling/domain';
import { useUpdateBookingStatus } from '~domains/scheduling/application/bookingMutations';
...
const BookingListItem = ({ item, classId, onSeeNotes }: BookingListItemProps) => {
	...
	const { mutate: updateStatus, isLoading: isUpdatingStatus, variables } =
			...
			<View style={styles.statusRow}>
				{ATTENDANCE_STATUSES.map(status => (
					<StatusButton
						...
						loading={isUpdatingStatus && variables?.status === status}
